...
# --- GMP / MPFR sanity ---
# Accept either ${prefix}/include/<header>.h or a multiarch layout where the
# header lives under ${prefix}/include/<triplet>/<header>.h and is reachable
# via $CPATH (which env_sukimasim.sh sets for the multiarch case).
_have_header() {
  local header="$1"
  local prefix="$2"
  [ -e "${prefix}/include/${header}" ] && return 0
  ls "${prefix}/include/"*"-linux-gnu/${header}" >/dev/null 2>&1 && return 0
  return 1
}

need_dev_pkg=0
if [ -z "${GMP_DIR:-}" ] || ! _have_header gmp.h "${GMP_DIR}"; then
  echo "[ERROR] gmp.h not found under ${GMP_DIR:-<GMP_DIR unset>}/include or its multiarch subdir." >&2
  echo "        Debian/Ubuntu: install libgmp-dev (sudo apt-get install libgmp-dev), then re-source local/env_sukimasim.sh" >&2
  need_dev_pkg=1
fi
if [ -z "${MPFR_DIR:-}" ] || ! _have_header mpfr.h "${MPFR_DIR}"; then
  echo "[ERROR] mpfr.h not found under ${MPFR_DIR:-<MPFR_DIR unset>}/include or its multiarch subdir." >&2
  echo "        Debian/Ubuntu: install libmpfr-dev (sudo apt-get install libmpfr-dev), then re-source local/env_sukimasim.sh" >&2
  need_dev_pkg=1
fi
...
# DPI_DLLESPEC is a Mentor/QuestaSim svdpi.h extension (Windows __declspec on
# their build, empty everywhere else). sukimasim's bundled svdpi.h does not
# define it, but include/dpiheader.h was autogenerated by QuestaSim and uses
# it on every prototype, so we override CXX to inject `-DDPI_DLLESPEC=` for a
# no-op expansion on Linux. Same trick avoids editing the upstream Makefile.
make CXX='g++ -DDPI_DLLESPEC=' \
     TOOL="${TOOL}" GEN_PATH="${GEN_PATH}" GMP_DIR="${GMP_DIR}" MPFR_DIR="${MPFR_DIR}" \
     2>&1 | tee "${PROJECT_DIR}/local/refmodel_build.log"
